...
פרס: מנויים פעילים → הכנסות − תפעול → 50% לקופה → 4 שבועות; 5 זוכים/שבוע; מינימום 7 חתימות/חודש לזכאות.
...
## קופת פרסים (ברירת מחדל — עדכון חי מ-prize_config בנתונים)
הכנסות = מספר premium × monthly_price_ils
הוצאות תפעול = operational_cost_pct% מההכנסות (או סכום בפועל מ-operational_costs)
רווח נקי = הכנסות − תפעול
קופה חודשית = prize_pool_pct% מהרווח הנקי (ברירת מחדל 50%)
קופה שבועית ≈ חודשי/4 פחות withholding_tax_pct

תנאי זכאות שבוע: מקום ב-5 הראשונים בניקוד; לפחות 7 חתימות בחודש; חתימות מאומתות בשרת בלבד.
שוויון ניקוד: מי קרוב יותר ל-DNA שלו מנצח.
